...
- a subscriber to read the tables from Tabsdata and write the modified data to an external system.
 ...
 If you encounter any trouble with the tutorial, you might find help in [Troubleshooting](../troubleshooting#troubleshooting), and reach out to us on [Slack](https://join.slack.com/t/tabsdata-community/shared_invite/zt-322toyigx-ZGFioMV2Gbza4bJDAR7wSQ).

  
## A Note on Tabsdata UI

 Once you have started the Tabsdata server using CLI, you can use the Tabsdata UI to work with the platform. The following section on **Getting Started** covers in detail the basic capabilities of the UI including e.g. executing functions or checking table data. You can also see a walkthrough of the UI [here](../user-interface#ui).
...
## Step 4. Log in and Create a Collection
 ...
 Collections are logical containers for Tabsdata [tables](../tables/table-frame#working-with-tables) and [functions](../04-working-with-functions#working-with-functions). You use collections to enable different business domains to have their own organizational space.
...
```
 ...
 You can configure publishers to read data from many external systems. For details, [Publishers](../publishers#working-with-publishers).
...
- Writes country-specific data to the appropriate table
 ...
 Since no trigger is explicitly defined, the transformer is triggered by a commit to the specified input table: `persons`. You can use a `trigger_by` command to define different trigger tables or to prevent automated triggering. For more information, see [Working with Triggers](../triggers#working-with-triggers).

 You can configure transformers to perform a range of operations. For details, see [Transformers](../transformers#working-with-transformers).
...
#### Using UI
 ...
 You can also use UI to trigger the transformer in the same way as shown for publisher here.
  ...
  If you face any issues, make sure to check out the [Troubleshooting guide](../troubleshooting#troubleshooting).
...
```
 ...
 You can configure subscribers to export data to many external systems. For details, see [Working with Triggers](../triggers#working-with-triggers).
...
## Step 8. Initiate Automated Triggers
 ...
 As mentioned earlier, transformer and subscriber functions have default automated triggers: when a trigger is not explicitly defined, the input tables for the function act as trigger tables. Publishers do not include a default trigger like transformers and subscribers, but you can define them when needed. For more information, see [Working with Triggers](../triggers#working-with-triggers).
...
## Step 9. Check the lineage graph
 ...
 You can check the lienage graph in the Tabsdata browser UI. You can see more details on [User Interface](../user-interface#ui).
...
## Next Steps
 ...
 - For a broader understanding of Tabsdata, see [Key Concepts](../key-concepts#key-concepts).
- Learn more about defining [publishers](../publishers#working-with-publishers), [transformers](../transformers#working-with-transformers), and [subscribers](../subscribers#working-with-subscribers).
- Dive deeper into the processing that you can do with transformers [here](../tables/table-frame#working-with-tables).
- Want to access an external system that does not have built-in support? You can build your own [connector plugins](../plugins#working-with-connector-plugins).
- If you run into problems, check out our [Troubleshooting](../troubleshooting#troubleshooting) chapter.
