...
You can use this built-in Tabsdata publisher to read files from Google Cloud Storage to the Tabsdata server. Publishers can read the following file formats: CSV, jsonl, ndjson, parquet, AVRO, and log (also read Grok for Log Files).

## Example (Publisher - Google Cloud Storage)
...
gcp_credentials = td.GCPServiceAccountKeyCredentials(
    ...
    account_name=td.HashiCorpSecret("path-to-secret","GCP_SERVICE_ACCOUNT_KEY"),
    )
...
## Setup (Publisher - Google Cloud Storage)

The following code uses placeholder values for defining a publisher that reads data from Google Cloud Storage and publishes it to Tabsdata tables:
...
A publisher needs credentials to read files from Google Cloud Storage.
...
```python
source=td.GCSSource(["gs://<path_to_file1.ext>","gs://<path_to_file2.ext>"], gcp_credentials),
```
...
```python
source=td.GCSSource([
        "gs://<path_to_file1.csv>",
        "gs://<path_to_file2.csv>",
