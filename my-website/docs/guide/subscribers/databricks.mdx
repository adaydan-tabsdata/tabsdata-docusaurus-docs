...
The following code uses placeholder values for defining a subscriber that reads Tabsdata tables and writes to Databricks:
...
For a Databricks subscriber with `if_table_exists=replace` (`schema_strategy` is ignored), the table is dropped and recreated on each execution. The resulting schema exactly matches the newly exported data, and type changes are fully reflected in the new table schema.

For a Databricks subscriber with `if_table_exists=append` and `schema_strategy=update`, the existing table schema is augmented to include new columns. Columns that are no longer present in the source remain in the table.

For a Databricks subscriber with `if_table_exists=append` and `schema_strategy=strict`, any schema changes (added, removed, or modified columns) will cause execution to fail.
