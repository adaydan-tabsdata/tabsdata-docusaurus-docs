...
`<input_table_1>` is the name of an input table in the collection that the function is registered with. `<collection_name>/<input_table_2>` is an input table from a collection that the transformer is not registered with.
