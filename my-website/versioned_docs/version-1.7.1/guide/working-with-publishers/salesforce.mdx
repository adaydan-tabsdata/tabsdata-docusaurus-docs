...
The following example creates a publisher named `read_sales`. This publisher runs two queries on Salesforce tables and writes the results of the query to two Tabsdata tables. There is a data quality check on the `contacts` table. It filters out rows where `AccountId` is NULL, routing them to a separate `clean_nulls` quarantine table. The main `contacts` table output only contains rows with valid (non-NULL) Account IDs.
