...
You can use this built-in Tabsdata publisher to read from a MariaDB database.
...
def read_sales(tf1: td.TableFrame, tf2: td.TableFrame):
...
## Setup (Publisher - MariaDB)

The following code uses placeholder values for defining a publisher that reads data from a MariaDB database and publishes it to Tabsdata tables:
...
def <publisher_name>(<table_frame1>:td.TableFrame, <table_frame2>:td.TableFrame):
    <function_logic>
    return <table_frame_output1>, <table_frame_output2>, {"<variable_name1>": <new_value1>, "<variable_name2>": <new_value2>}
```
