...
The following example creates a publisher named `read_contacts`. This publisher retreives a report from Salesforce, filters on certain parameters including last modification time and writes the result to a Tabsdata table. There is a data quality check on the `contacts` table. It filters out rows where `AccountId` is NULL, routing them to a separate `clean_nulls` quarantine table. The main `contacts` table output only contains rows with valid (non-NULL) Account IDs.
...
The report reference by `id` or `name` can be explicity stated using this optional parameter. If not provided, it will be inferred from the value of report parameter. If using find_report_by=’name’ the name is what salesforce refers to as the report unique name or the developer name.
...
This property defines the relationship between various filters, if multiple filters are provided. The relationship in defined using filter index, where first filter in the list of filters defined in `filter` property is referred as 1, the second filter in the list is referred as 2 and so on. Hence, the `filter_logic` value would be for example “1 OR 2”, or “1 OR (2 AND 3)” etc.

#### [Optional] initial_last_modified

The value of starting date for incremental publishing is defined here. If provided, it must be a string in Salesforce datetime format (with informed timezone).

#### [Optional] last_modified_column

The property to use for incremental loading based on the last modified date. If provided, the report must contain this column. Defaults to None.
