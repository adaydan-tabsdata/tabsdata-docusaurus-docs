...
You can use a publisher to read from a MariaDB database.
...
**Database Credentials**: `<db_username>` and `<db_password>` are the username and password to log in to the database respectively. You can use different ways to store the credentials which are highlighted here in the documentation.

**Initial Values**:

[optional] `<variable_name1>`, `<variable_name2>`,… are the variable names included in the database queries.

[optional] `<variable_value1>`, `<variable_value2>`,… are the values to assign to the defined variables.
