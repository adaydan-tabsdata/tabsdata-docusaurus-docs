...
You can use this built-in Tabsdata publisher to read files from Amazon S3 to the Tabsdata server. Publishers can read the following file formats: CSV, jsonl, ndjson, parquet, and log.
...
A publisher may receive and return a `None` value instead a TableFrame parameter or return value.
